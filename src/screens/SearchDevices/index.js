...
import {
  askLocationPermissionForBLEScan,
...
const SearchDevices = () => {
  ...
  const handleDiscoverPeripheral = peripheral => {
    ...
    store.setBLEPeripheral(peripheral.id, peripheral);
  };
    ...
    try {
      ...
      if (!item.isConnected) {
        ...
        if (value) {
          ...
          throw new Error(
            'Connecting to device failed. Try searching again and connect',
          );
        }
      }
...
const EmptyComponent = () => {
  return (
    <Column mt={20}>
      <Heading textAlign={'center'} size={'md'} color={'#'}>
        No devices Found
      </Heading>
      <ActionButton alignSelf={'center'} mt={3}>
        Search Again
      </ActionButton>
    </Column>
  );
