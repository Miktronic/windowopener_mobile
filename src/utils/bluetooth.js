...
export async function checkBluetoothPermission() {
  if (Platform.OS === 'ios') {
    ...
    for (let i = 0; i < statuses.length; i++) {
      if (
        statuses[i] === RESULTS.DENIED ||
        statuses[i] === RESULTS.UNAVAILABLE ||
        statuses[i] === RESULTS.BLOCKED
      ) {
        return false;
      }
    }
...
export async function connectPeripheral(peripheralId) {
  ...
  if (!isConnected) {
    ...
    if (value) {
      ...
      throw new Error(
        'Connecting to device failed. Try turning on both bluetooth and location and try searching again',
      );
    }
  }
}
